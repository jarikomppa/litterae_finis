...
static void ZB_copyFrameBufferRGB32(ZBuffer * zb,
                                    ...
                                    int linesize)
{
	memcpy(buf,zb->pbuf,linesize*zb->ysize*4);
/*
    unsigned short *q;
 ...
 */
void memset_s16(void *adr, int val, int count)
{
    int i, n, v;
    unsigned int *p;
    unsigned short *q;
...
void memset_s32(void *adr, int val, int count)
{
    int i, n, v;
    unsigned int *p;
	      ...
	      int clear_color, int r, int g, int b)
{
    ...
    if (clear_z) {
	memset_s16(zb->zbuf, z, zb->xsize * zb->ysize);
    }
    if (clear_color) {
	pp = zb->pbuf;
    color = RGB_TO_PIXEL(r, g, b);
	for (y = 0; y < zb->ysize; y++) {
	    memset_s32(pp, color, zb->xsize);
